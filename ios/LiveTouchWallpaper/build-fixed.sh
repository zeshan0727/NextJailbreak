...
OUT="LiveTouch-0.1.6-roothide-ellekit-fix.tipa"
rm -rf "$ROOT/build" "$ROOT/Payload"
mkdir -p "$ROOT/build"

# TrollStore app/helper stay normal so they have no launch-time libroothide
# dependency. The SpringBoard engine is RootHide-built for ElleKit injection.
...
d['CFBundleShortVersionString']='0.1.6'
d['CFBundleVersion']='7'
...
strings "$ROOT/Payload/LiveTouch.app/LiveTouch" | grep -Fq 'RootHide active injection directory:'
strings "$ROOT/Payload/LiveTouch.app/LiveTouch" | grep -Fq 'Engine installed for RootHide/ElleKit.'
strings "$ROOT/Payload/LiveTouch.app/LiveTouchRootHelper" | grep -Fq 'RootHide active injection directory:'
strings "$ROOT/Payload/LiveTouch.app/LiveTouchEngine.dylib" | grep -Fq 'LiveTouchEngine 0.1.6 loaded into'
strings "$ROOT/Payload/LiveTouch.app/LiveTouchEngine.dylib" | grep -Fq 'CSCoverSheetViewController'
strings "$ROOT/Payload/LiveTouch.app/LiveTouchEngine.dylib" | grep -Fq 'engine.log'
plutil -lint "$ROOT/Payload/LiveTouch.app/LiveTouchEngine.plist" >/dev/null
