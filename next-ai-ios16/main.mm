...
@implementation ChatController {
 ...
 BOOL _busy; BOOL _savingHistory; UIStackView *_suggestions; std::atomic_bool _cancel;
}
...
- (void)viewDidLoad {
 [super viewDidLoad]; self.title=@"Next AI"; self.view.backgroundColor=NABackground();
 _savingHistory=[NSUserDefaults.standardUserDefaults boolForKey:@"saveChats"];
 ...
 if(_savingHistory && [NSUserDefaults.standardUserDefaults boolForKey:@"resumeChat"]){NSString *last=[NSUserDefaults.standardUserDefaults stringForKey:@"lastChatID"];for(NSDictionary *c in _archive)if([c[@"id"] isEqual:last]){_chatID=c[@"id"];_messages=[c[@"messages"] mutableCopy];break;}}
 ...
 _output=[UITextView new]; _output.editable=NO; _output.font=[UIFont systemFontOfSize:17]; _output.backgroundColor=UIColor.secondarySystemBackgroundColor; _output.layer.cornerRadius=22; _output.textContainerInset=UIEdgeInsetsMake(22,16,22,16);_output.keyboardDismissMode=UIScrollViewKeyboardDismissModeInteractive;
 _input=[UITextField new]; _input.placeholder=@"Message your offline AI"; _input.borderStyle=UITextBorderStyleRoundedRect;_input.accessibilityLabel=@"Chat message";
 ...
 NAButton(_send,@"Send",@"arrow.up");
 [_send.widthAnchor constraintEqualToConstant:94].active=YES;
 _suggestions=[UIStackView new];_suggestions.spacing=8;_suggestions.distribution=UIStackViewDistributionFillEqually;
 for(NSString *title in @[@"Write",@"Explain",@"Ideas"]){UIButton *b=[UIButton buttonWithType:UIButtonTypeSystem];UIButtonConfiguration *c=[UIButtonConfiguration tintedButtonConfiguration];c.title=title;c.baseForegroundColor=NAAccent();c.cornerStyle=UIButtonConfigurationCornerStyleCapsule;b.configuration=c;[b addTarget:self action:@selector(suggestion:) forControlEvents:UIControlEventTouchUpInside];[_suggestions addArrangedSubview:b];}
 UIStackView *stack=[[UIStackView alloc] initWithArrangedSubviews:@[_status,_output,_suggestions,row]]; stack.axis=UILayoutConstraintAxisVertical; stack.spacing=12; stack.translatesAutoresizingMaskIntoConstraints=NO; [self.view addSubview:stack];
 [NSLayoutConstraint activateConstraints:@[[stack.topAnchor constraintEqualToAnchor:self.view.safeAreaLayoutGuide.topAnchor constant:12],[stack.leadingAnchor constraintEqualToAnchor:self.view.leadingAnchor constant:16],[stack.trailingAnchor constraintEqualToAnchor:self.view.trailingAnchor constant:-16],[stack.bottomAnchor constraintEqualToAnchor:self.view.keyboardLayoutGuide.topAnchor constant:-12],[row.heightAnchor constraintEqualToConstant:50]]];
 self.navigationItem.rightBarButtonItem=[[UIBarButtonItem alloc] initWithTitle:@"Options" style:UIBarButtonItemStylePlain target:self action:@selector(menu)];
 ...
 self.navigationItem.leftBarButtonItem=[[UIBarButtonItem alloc] initWithImage:[UIImage systemImageNamed:@"square.and.pencil"] style:UIBarButtonItemStylePlain target:self action:@selector(newChat)];self.navigationItem.leftBarButtonItem.accessibilityLabel=@"New chat";
 [self refresh];
}
- (void)viewWillAppear:(BOOL)animated {[super viewWillAppear:animated];BOOL saving=[NSUserDefaults.standardUserDefaults boolForKey:@"saveChats"];if(saving!=_savingHistory && !_busy){_savingHistory=saving;_messages=[NSMutableArray new];_chatID=NSUUID.UUID.UUIDString;}if(!_busy)[self refresh];}
- (void)traitCollectionDidChange:(UITraitCollection *)previous {[super traitCollectionDidChange:previous];if(_output && !_busy)[self refresh];}
- (void)newChat {if(NextAIWorking){[self alert:@"Finish the current operation first."];return;}[self save];_messages=[NSMutableArray new];_chatID=NSUUID.UUID.UUIDString;[NSUserDefaults.standardUserDefaults removeObjectForKey:@"lastChatID"];[self refresh];}
- (void)suggestion:(UIButton *)b {NSDictionary *prompts=@{@"Write":@"Help me write a friendly message about ",@"Explain":@"Explain in simple terms how ",@"Ideas":@"Give me five creative ideas for "};_input.text=prompts[b.currentTitle] ?: @"";[_input becomeFirstResponder];}
- (void)background { _cancel=true; [self save]; }
- (void)refresh {
 BOOL memory=[NSUserDefaults.standardUserDefaults boolForKey:@"memoryEnabled"];
 BOOL saved=[NSUserDefaults.standardUserDefaults boolForKey:@"saveChats"];
 _status.text=_model ? [NSString stringWithFormat:@"●  ON DEVICE   ·   %@   ·   %@",saved?@"Saved chat":@"Temporary chat",memory?@"Memory on":@"Memory off"] : @"●  PRIVATE BY DESIGN   ·   Add a chat model in Options";
 _status.textColor=NAAccent();_suggestions.hidden=_messages.count>0;
 ...
 for(NSDictionary *m in _messages){BOOL user=[m[@"role"] isEqual:@"user"];NSString *name=user?@"YOU":@"NEXT AI";
 [text appendAttributedString:[[NSAttributedString alloc] initWithString:[name stringByAppendingString:@"\n"] attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:11 weight:UIFontWeightBold],NSForegroundColorAttributeName:user?NAColor(0xd86c36):NAAccent(),NSKernAttributeName:@1.5}]];
 NSMutableParagraphStyle *paragraph=[NSMutableParagraphStyle new];paragraph.lineSpacing=5;paragraph.paragraphSpacing=14;
 [text appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@\n\n",m[@"content"]] attributes:@{NSFontAttributeName:[UIFont preferredFontForTextStyle:UIFontTextStyleBody],NSForegroundColorAttributeName:UIColor.labelColor,NSParagraphStyleAttributeName:paragraph}]];}
 if(!text.length){text=[[NSMutableAttributedString alloc] initWithString:@"Your ideas.\nOn your iPhone.\n\nWrite something new, explore a question, or find your next idea.\n\nPrivate conversations. Offline intelligence.\n\nChoose a prompt below, or add your model from Options to begin." attributes:@{NSFontAttributeName:[UIFont preferredFontForTextStyle:UIFontTextStyleBody],NSForegroundColorAttributeName:UIColor.secondaryLabelColor}];[text addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:30 weight:UIFontWeightBold],NSForegroundColorAttributeName:UIColor.labelColor} range:NSMakeRange(0,27)];}
 _output.attributedText=text;
 if(_messages.count && follow && _output.text.length) [_output scrollRangeToVisible:NSMakeRange(_output.text.length-1,1)];
}
...
- (void)save {
 if(!_messages.count || ![NSUserDefaults.standardUserDefaults boolForKey:@"saveChats"]) return;
 [NSUserDefaults.standardUserDefaults setObject:_chatID forKey:@"lastChatID"];
...
- (void)menu {
 ...
 [a addAction:[UIAlertAction actionWithTitle:@"New chat" style:UIAlertActionStyleDefault handler:^(UIAlertAction *x){[self newChat];}]];
 ...
 [a addAction:[UIAlertAction actionWithTitle:@"Share conversation" style:UIAlertActionStyleDefault handler:^(UIAlertAction *x){if(!self->_messages.count){[self alert:@"Start a conversation first."];return;}UIActivityViewController *share=[[UIActivityViewController alloc] initWithActivityItems:@[self->_output.text] applicationActivities:nil];share.popoverPresentationController.barButtonItem=self.navigationItem.rightBarButtonItem;[self presentViewController:share animated:YES completion:nil];}]];
 [a addAction:[UIAlertAction actionWithTitle:@"Retry last message" style:UIAlertActionStyleDefault handler:^(UIAlertAction *x){if(!self->_messages.count)return;if([self->_messages.lastObject[@"role"] isEqual:@"assistant"])[self->_messages removeLastObject];if([self->_messages.lastObject[@"role"] isEqual:@"user"]){self->_input.text=self->_messages.lastObject[@"content"];[self->_messages removeLastObject];[self send];}}]];
 [a addAction:[UIAlertAction actionWithTitle:@"Memory & settings" style:UIAlertActionStyleDefault handler:^(UIAlertAction *x){self.tabBarController.selectedIndex=2;}]];
...
- (void)chooseLocalModel {
 ...
 if(error || size.unsignedLongLongValue>2200000000ULL){[self alert:error.localizedDescription ?: @"Choose a model smaller than 2.2 GB."];return;}
...
- (void)showChats {
 ...
 [a addAction:[UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(UIAlertAction *x){self->_chatID=[NSUserDefaults.standardUserDefaults boolForKey:@"saveChats"]?c[@"id"]:NSUUID.UUID.UUIDString; self->_messages=[c[@"messages"] mutableCopy]; [self refresh];}]]; }
 [a addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]]; a.popoverPresentationController.barButtonItem=self.navigationItem.rightBarButtonItem; [self presentViewController:a animated:YES completion:nil];
}
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
 ...
 if(error) return;
 if(size.unsignedLongLongValue>2200000000ULL) {error=[NSError errorWithDomain:@"NextAI" code:1 userInfo:@{NSLocalizedDescriptionKey:@"Choose a model smaller than 2.2 GB. Use the recommended Qwen Q4_K_M."}];return;}
...
- (void)send {
 ...
 if(!_model){[self alert:@"Import a model from Options first."];return;}
 if(input.length>6000){[self alert:@"Please shorten the message to fit the local model context."];return;}
 [_messages addObject:@{@"role":@"user",@"content":input}]; _input.text=@""; [self save];
 NSString *memory=[NSUserDefaults.standardUserDefaults boolForKey:@"memoryEnabled"]?([NSUserDefaults.standardUserDefaults stringForKey:@"memoryNotes"] ?: @""):@"";
 NSString *systemPrompt=@"You are Next AI, a helpful offline assistant. Be clear and concise. Answer in the user's language. Never claim to have saved memory: users manage persistent notes in Settings.";
 if(memory.length)systemPrompt=[systemPrompt stringByAppendingFormat:@"\nUser-provided preferences and facts (may be outdated; the current request takes priority):\n%@",memory];
 NSArray *history=[_messages copy]; [_messages addObject:@{@"role":@"assistant",@"content":@""}];
 NextAIWorking=YES;_busy=YES;_cancel=false;UIApplication.sharedApplication.idleTimerDisabled=YES;_input.enabled=NO;NAButton(_send,@"Stop",@"stop.fill");[self refresh];_status.text=@"Loading model on device…";
 ...
 roles.push_back("system");contents.emplace_back(systemPrompt.UTF8String);
 ...
 int n=llama_chat_apply_template(tmpl,chat.data(),chat.size(),true,nullptr,0);if(n<=0)throw std::runtime_error("This model's chat template is not supported by this app.");
 ...
 if(needed<1 || needed>1664)throw std::runtime_error("This conversation is full. Start a new chat or shorten your saved memory and message.");
 ...
 NextAIWorking=NO;UIApplication.sharedApplication.idleTimerDisabled=NO;self->_busy=NO;self->_input.enabled=YES;NAButton(self->_send,@"Send",@"arrow.up");
 if(![self->_messages.lastObject[@"content"] length]) [self->_messages removeLastObject];
 [self refresh];[self save]; if(failure)[self alert:failure];else if(self->_cancel)self->_status.text=@"Stopped";else if(limit)self->_status.text=@"Reply length limit reached";
 });
 }});
}
@end
...
- (BOOL)application:(UIApplication *)app didFinishLaunchingWithOptions:(NSDictionary *)options {
 [NSUserDefaults.standardUserDefaults registerDefaults:@{@"saveChats":@YES,@"resumeChat":@YES,@"memoryEnabled":@NO,@"appearance":@0}];
 UINavigationBarAppearance *nav=[UINavigationBarAppearance new];[nav configureWithOpaqueBackground];nav.backgroundColor=NABackground();nav.shadowColor=UIColor.clearColor;[UINavigationBar appearance].standardAppearance=nav;[UINavigationBar appearance].scrollEdgeAppearance=nav;
 UITabBarAppearance *bar=[UITabBarAppearance new];[bar configureWithOpaqueBackground];bar.backgroundColor=NABackground();[UITabBar appearance].standardAppearance=bar;[UITabBar appearance].scrollEdgeAppearance=bar;
 self.window=[[UIWindow alloc] initWithFrame:UIScreen.mainScreen.bounds];UINavigationController *chat=[[UINavigationController alloc] initWithRootViewController:[ChatController new]];chat.tabBarItem=[[UITabBarItem alloc] initWithTitle:@"Chat" image:[UIImage systemImageNamed:@"bubble.left.and.bubble.right"] tag:0];UINavigationController *photos=[[UINavigationController alloc] initWithRootViewController:[PhotoController new]];photos.tabBarItem=[[UITabBarItem alloc] initWithTitle:@"Create" image:[UIImage systemImageNamed:@"photo"] tag:1];UITabBarController *tabs=[UITabBarController new];UINavigationController *settings=[[UINavigationController alloc] initWithRootViewController:[SettingsController new]];settings.tabBarItem=[[UITabBarItem alloc] initWithTitle:@"Settings" image:[UIImage systemImageNamed:@"slider.horizontal.3"] tag:2];tabs.viewControllers=@[chat,photos,settings];NAApplyTheme(self.window);self.window.rootViewController=tabs;[self.window makeKeyAndVisible];return YES;
}
@end
