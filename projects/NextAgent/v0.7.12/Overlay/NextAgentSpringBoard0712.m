...
static NSDictionary *NAHIDPastePayload(void) {
    ...
    return @{
        @"success": @YES,
        @"bridge_version": @"0.7.12",
        @"path": @"springboard_iohid_v0712_cmd_v"
    };
}
...
static NSDictionary *NAHIDTypeTextPayload(NSString *text) {
    ...
    return @{
        @"success": @YES,
        @"bridge_version": @"0.7.12",
        @"path": @"springboard_iohid_v0712_direct_text",
        @"typed_characters": @(typed)
    };
}
...
- (BOOL)showSystemHUD;
- (void)hideSystemHUD;
- (void)updateSystemHUDMode:(NSString *)mode message:(NSString *)message;
...
- (void)ensureSystemHUD {
    if (self.systemHUDController || self.systemHUDAvailable) return;
    ...
    @try {
        ...
        if (workspaceClass && [workspaceClass respondsToSelector:ifExists]) {
            workspace = ((id (*)(id, SEL))objc_msgSend)((id)workspaceClass, ifExists);
        }
        if (!workspace && workspaceClass && [workspaceClass respondsToSelector:shared]) {
            workspace = ((id (*)(id, SEL))objc_msgSend)((id)workspaceClass, shared);
        }

        SEL hudSelector = NSSelectorFromString(@"HUDController");
        id hudController = (workspace && [workspace respondsToSelector:hudSelector])
            ? ((id (*)(id, SEL))objc_msgSend)(workspace, hudSelector)
            : nil;
        SEL sessionSelector = NSSelectorFromString(@"HUDSessionForViewController:identifier:");
        SEL presentSelector = NSSelectorFromString(@"_presentHUD:animated:");
        SEL dismissSelector = NSSelectorFromString(@"_dismissHUD:animated:");
        if (!hudController ||
            ![hudController respondsToSelector:sessionSelector] ||
            ![hudController respondsToSelector:presentSelector] ||
            ![hudController respondsToSelector:dismissSelector]) {
        ...
        controller.view.frame = CGRectMake(0, 0, 238.0, 48.0);
        ...
        pill.backgroundColor = [UIColor colorWithWhite:0.035 alpha:0.95];
        pill.layer.cornerRadius = 20.0;
        pill.layer.borderWidth = 0.65;
        pill.layer.borderColor = [UIColor colorWithWhite:1.0 alpha:0.16].CGColor;
        pill.userInteractionEnabled = NO;
        [controller.view addSubview:pill];

        UIActivityIndicatorView *spinner = [[UIActivityIndicatorView alloc]
            initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleMedium];
        spinner.frame = CGRectMake(10, 10, 20, 20);
        ...
        UILabel *icon = [[UILabel alloc] initWithFrame:CGRectMake(10, 9, 20, 22)];
        ...
        UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(34, 4, 188, 32)];
        ...
        id session = ((id (*)(id, SEL, id, id))objc_msgSend)(
            ...
            sessionSelector,
            controller,
            @"uk.zeshanbarvi.nextagent.status.v0712"
        );
        if (!session) {
            self.systemHUDAvailable = NO;
            return;
        ...
        self.systemHUDTitle = title;
        self.systemHUDIcon = icon;
        self.systemHUDSpinner = spinner;
...
- (BOOL)showSystemHUD {
    ...
    if (!self.systemHUDAvailable || !self.systemHUDController || !self.systemHUDSession) {
        return NO;
    }
    @try {
        if (!self.systemHUDVisible) {
            SEL selector = NSSelectorFromString(@"_presentHUD:animated:");
...
- (void)hideSystemHUD {
    if (!self.systemHUDVisible || !self.systemHUDController || !self.systemHUDSession) return;
    @try {
        SEL selector = NSSelectorFromString(@"_dismissHUD:animated:");
        if ([self.systemHUDController respondsToSelector:selector]) {
            ((void (*)(id, SEL, id, BOOL))objc_msgSend)(
                self.systemHUDController,
                selector,
                self.systemHUDSession,
...
- (void)updateSystemHUDMode:(NSString *)mode message:(NSString *)message {
    [self ensureSystemHUD];
    if (!self.systemHUDAvailable) return;

    NSString *text = message.length ? message : @"Next Agent working";
    self.systemHUDTitle.text = text;

    if ([mode isEqualToString:@"working"]) {
        self.systemHUDSpinner.hidden = NO;
        self.systemHUDIcon.hidden = YES;
        [self.systemHUDSpinner startAnimating];
        return;
    }

    [self.systemHUDSpinner stopAnimating];
    self.systemHUDSpinner.hidden = YES;
    self.systemHUDIcon.hidden = NO;
    if ([mode isEqualToString:@"complete"]) {
        self.systemHUDIcon.text = @"✓";
        self.systemHUDIcon.textColor = [UIColor colorWithRed:0.26 green:0.96 blue:0.64 alpha:1.0];
    } else if ([mode isEqualToString:@"stopped"]) {
        self.systemHUDIcon.text = @"■";
        self.systemHUDIcon.textColor = [UIColor colorWithRed:1.0 green:0.38 blue:0.47 alpha:1.0];
    } else {
        self.systemHUDIcon.text = @"!";
        self.systemHUDIcon.textColor = [UIColor colorWithRed:1.0 green:0.38 blue:0.47 alpha:1.0];
    }
...
- (void)setCapturePresentationSuppressed:(BOOL)suppressed {
    self.pill.hidden = suppressed;
    self.systemHUDPill.hidden = suppressed;
}
...
- (NSDictionary *)directCapturePayload {
    ...
    if (!image) {
        return @{
            ...
            @"transport_version": @"0.7.12-cfmessageport1"
        };
    }
...
- (NSDictionary *)directOCRPayloadFast:(BOOL)fast
    ...
    if (!image || !image.CGImage) {
        return @{
            ...
            @"quality": quality ?: @{},
            @"error": @"SpringBoard compositor did not return a validated frame",
            @"transport_version": @"0.7.12-springboard-vision1"
        };
    }
...
- (void)refresh {
    ...
    [self updateSystemHUDMode:mode message:(message.length ? message : @"Next Agent working")];
    [self show];
    ...
    if ([mode isEqualToString:@"complete"]) {
        ...
        [self updateSystemHUDMode:@"complete" message:(message.length ? message : @"Complete")];
    ...
    [self updateSystemHUDMode:mode message:self.titleLabel.text];
    [self setProgress:1.0 animated:YES];
...
- (NSDictionary *)close {
    ...
    self.opening = NO;
    self.lastOpenResult = nil;
    return @{@"success": @YES, @"active": @NO, @"bridge_version": @"0.7.12"};
}
...
static CFStringRef const NABridgeServiceName =
    CFSTR("uk.zeshanbarvi.nextagent.bridge.v0712");
    ...
    @autoreleasepool {
        ...
        if ([action isEqualToString:@"ping"]) {
            return NABridgeReply(@{
                ...
                @"split_workspace": @"target_scene_overlay_50_50_async",
                @"hid": @"springboard_iohid_v0712"
            });
        }
