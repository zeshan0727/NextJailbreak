...
- (NSDictionary *)directCapturePayload {
    ...
    if (!image) {
        return @{
            ...
            @"transport_version": @"0.7.11-cfmessageport1"
        };
    }
...
- (NSDictionary *)directOCRPayloadFast:(BOOL)fast
    ...
    if (!image || !image.CGImage) {
        return @{
            ...
            @"quality": quality ?: @{},
            @"error": @"SpringBoard compositor did not return a validated frame",
            @"transport_version": @"0.7.11-springboard-vision1"
        };
    }
...
- (NSDictionary *)close {
    ...
    self.opening = NO;
    self.lastOpenResult = nil;
    return @{@"success": @YES, @"active": @NO, @"bridge_version": @"0.7.11"};
}
...
static CFStringRef const NABridgeServiceName =
    CFSTR("uk.zeshanbarvi.nextagent.bridge.v0711");
...
: *usage = 0x21u; *shift = YES; return YES;
        case '%': *usage = 0x22u; *shift = YES; return YES;
        case '^': *usage = 0x23u; *shift = YES; return YES;
        case '&': *usage = 0x24u; *shift = YES; return YES;
        case '*': *usage = 0x25u; *shift = YES; return YES;
...
static NSDictionary *NAHIDTypeTextPayload(NSString *text) {
    ...
    for (NSUInteger i = 0; i < text.length; i++) {
        ...
        if (!NAHIDUsageForCharacter(c, &usage, &shift)) {
            return @{
                @"success": @NO,
                @"error": [NSString stringWithFormat:@"unsupported character at index %lu; use paste fallback", (unsigned long)i],
                @"typed_characters": @(typed),
                @"requires_paste_fallback": @YES
            };
        }
        ...
        BOOL down = NAHIDKeyboardEvent(usage, YES);
        usleep(12000);
        BOOL up = NAHIDKeyboardEvent(usage, NO);
        ...
        if (!down || !up) {
            return @{@"success": @NO, @"error": @"SpringBoard character event construction failed", @"typed_characters": @(typed)};
        }
    ...
    return @{
        @"success": @YES,
        @"bridge_version": @"0.7.11",
        @"path": @"springboard_iohid_admin_keys",
        @"typed_characters": @(typed)
    };
}
