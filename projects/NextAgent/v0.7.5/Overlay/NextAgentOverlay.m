...
typedef kern_return_t (*NACARenderServerRenderDisplay)(mach_port_t, CFStringRef, IOSurfaceRef, int, int);
...
- (UIImage *)captureDisplayWithSource:(NSString **)sourceOut {
    // iOS 16 render-server path: render the real compositor display into an
    // IOSurface. Do not use CARenderServerCaptureDisplay with a guessed
    // CGImage-returning signature; that can produce malformed/monochrome data.
    ...
    if (renderDisplay) {
        ...
        size_t width = (size_t)llround(screen.bounds.size.width * scale);
        size_t height = (size_t)llround(screen.bounds.size.height * scale);
        size_t bytesPerPixel = 4;
        size_t bytesPerRow = ((width * bytesPerPixel) + 63u) & ~((size_t)63u);
        ...
        if (surface) {
            kern_return_t kr = renderDisplay(0, NULL, surface, 0, 0);
            if (kr == KERN_SUCCESS) {
                ...
                BOOL looksBlack = YES;
                if (src && surfaceWidth >= 8 && surfaceHeight >= 8) {
                    uint32_t aggregate = 0;
                    for (int sy = 1; sy <= 4; sy++) {
                        size_t y = (surfaceHeight * (size_t)sy) / 5u;
                        const uint8_t *row = src + MIN(y, surfaceHeight - 1) * srcBPR;
                        for (int sx = 1; sx <= 8; sx++) {
                            size_t x = (surfaceWidth * (size_t)sx) / 9u;
                            const uint8_t *p = row + MIN(x, surfaceWidth - 1) * 4u;
                            aggregate |= (uint32_t)p[0] | (uint32_t)p[1] | (uint32_t)p[2];
                        }
                    }
                    looksBlack = (aggregate == 0);
                }
                ...
                if (src && !looksBlack) {
                    ...
                    if (context) {
                        ...
                        if (dst) {
                            size_t rowBytes = MIN(surfaceWidth * 4u, srcBPR);
                ...
                if (image) {
                    if (sourceOut) *sourceOut = @"render_server_iosurface";
                    return image;
                }
            } else {
                CFRelease(surface);
    ...
    // SpringBoard private UIKit fallback. This is intentionally executed in
    // SpringBoard, not in the backgrounded Next Agent application.
