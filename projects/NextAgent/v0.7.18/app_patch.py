...
s = s.replace('"client_version": "0.7.16"', '"client_version": "0.7.18"')
...
old_hud = '''        let hudRaw = NAScreenBridgeClient.hudStatus()
        ...
        results.append([
            ...
            "success": hudBridgeVersion == "0.7.18" && (nativeHUD || fallbackExpected),
...
ui = ui.replace(
    ...
    "Broadcasts a direct HUD test and verifies both injection and UIWindowScene readiness. 0.7.18 retries startup through app/scene lifecycle notifications so early TrollStore injection can no longer miss the scene."
)
...
y = project.read_text()
y = y.replace("MARKETING_VERSION: 0.7.16", "MARKETING_VERSION: 0.7.18")
y = y.replace("CURRENT_PROJECT_VERSION: 24", "CURRENT_PROJECT_VERSION: 26")
