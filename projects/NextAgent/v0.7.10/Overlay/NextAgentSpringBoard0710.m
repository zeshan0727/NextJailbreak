...
- (NSDictionary *)directCapturePayload {
    ...
    if (!image) {
        return @{
            ...
            @"transport_version": @"0.7.10-cfmessageport1"
        };
    }
...
- (NSDictionary *)directOCRPayloadFast:(BOOL)fast
    ...
    if (!image || !image.CGImage) {
        return @{
            ...
            @"quality": quality ?: @{},
            @"error": @"SpringBoard compositor did not return a validated frame",
            @"transport_version": @"0.7.10-springboard-vision1"
        };
    }
...
- (NSDictionary *)close {
    ...
    self.opening = NO;
    self.lastOpenResult = nil;
    return @{@"success": @YES, @"active": @NO, @"bridge_version": @"0.7.10"};
}
