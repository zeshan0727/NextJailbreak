...
static NSString * const NAForegroundHUDGeneration = @"foreground_app_hud_v0715";

@interface NAForegroundHUD : NSObject
...
@property(nonatomic,strong) NSDate *readyProbeUntil;
+ (instancetype)shared;
- (void)start;
- (void)refresh;
@end

@implementation NAForegroundHUD

+ (instancetype)shared {
    static NAForegroundHUD *value;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ value = [NAForegroundHUD new]; });
    return value;
}
...
- (NSString *)labelForDetail:(uint64_t)detail {
    switch (detail) {
        ...
        default: return [NSString stringWithFormat:@"Next Agent • Working"]; // foreground_app_hud_v0715
    }
}
...
- (void)refresh {
    ...
    if (mode == 2) {
        ...
        self.fill.backgroundColor = self.icon.textColor;
        self.label.text = mode == 4 ? @"Next Agent • Stopped" : @"Next Agent • Failed";
    }
    [self show];
}
...
- (void)start {
    ...
    notify_register_dispatch(
        ...
        ^(__unused int token) { [[NAForegroundHUD shared] refresh]; }
    );
    ...
    notify_register_dispatch(
        ...
        dispatch_get_main_queue(),
        ^(__unused int token) {
            NAForegroundHUD *hud = [NAForegroundHUD shared];
...
static void NAForegroundHUDInit(void) {
    @autoreleasepool {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (UIApplication.sharedApplication) {
                NAForegroundHUD *hud = [NAForegroundHUD shared];
                hud.stateToken = -1;
                if ([bundle isEqualToString:@"uk.zeshanbarvi.nextagent"]) {
                    hud.readyProbeUntil = [NSDate dateWithTimeIntervalSinceNow:1.5];
                }
                [hud start];
            }
        });
