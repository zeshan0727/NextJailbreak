...
static NSString * const NAStatusHUDGeneration = @"system_status_hud_v0717";
...
- (NSString *)labelForDetail:(uint64_t)detail {
    switch (detail) {
        ...
        default: return [NSString stringWithFormat:@"Next Agent HUD • Working"]; // system_status_hud_v0717
    }
}
...
#else
        // Generic UIApplication class-filter copy. The exact Next Agent and
        // known-app copies handle environments where RootHide/ElleKit does
        // not honor the Classes filter reliably.
        if ([bundle isEqualToString:@"uk.zeshanbarvi.nextagent"]) return;
#endif

        // Multiple filter copies may match a process. Use a process-global
        // associated-object marker keyed by a shared selector to keep one HUD.
        SEL guardKey = NSSelectorFromString(@"_nextAgentStatusHUD0717Loaded");
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (UIApplication.sharedApplication) {
                ...
                NAHUDWriteStatus(@{
                    ...
                    @"window_visible": @(hud.window != nil && !hud.window.hidden)
                });
            }
        });
    }
